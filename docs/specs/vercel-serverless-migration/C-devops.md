...
| **`@nestjs/schedule` (cron de resolución de disputas `@Cron(EVERY_HOUR)`)** | Los cron de Nest **no corren** en serverless (no hay proceso residente). → Proponer **Vercel Cron** (`crons` en `vercel.json` invocando un endpoint protegido con token admin), o mover la resolución a un GitHub Action horario. Requiere un endpoint HTTP nuevo. **Fuera de alcance**, solo se señala. |
